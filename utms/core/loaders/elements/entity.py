...
class EntityLoader(ComponentLoader[Entity, EntityManager]):
    ...
    def _get_entity_schema(self, entity_type_str: str) -> Optional[Dict[str, Any]]:
        if self.context and hasattr(self.context, 'current_entity_schema') and \
           self.context.current_entity_type == entity_type_str.lower():
            return self.context.current_entity_schema
        self.logger.warning(f"Schema for entity type '{entity_type_str}' not found in LoaderContext.")
        return None
    ...
    def parse_definitions(self, nodes: List[HyNode]) -> Dict[str, Dict[str, Any]]:
        ...
        for node in nodes:
            ...
            if node.type != expected_node_type: continue

            entity_instance_name = str(node.value)
            node_entity_type_str_from_plugin = getattr(node, "entity_type_name_str", None)
            if node_entity_type_str_from_plugin != current_entity_type_from_context: continue
    ...
    def create_object(
        ...
        global_variables_for_evaluation = self.context.variables if self.context and self.context.variables else {}
        
        entity_schema = self._get_entity_schema(entity_type_name_str)
        if not entity_schema:
            self.logger.error(f"Schema not found for '{entity_type_name_str}'. Default value processing will fail.")
            entity_schema = {} 
        ...
        for attr_name, initial_typed_value in initial_attributes_typed.items():
            ...
            if not initial_typed_value.is_dynamic:
                ...
                context_for_dynamic_service = {**global_variables_for_evaluation, "self": self_object_for_eval}
                raw_hy_object_to_evaluate = initial_typed_value._raw_value
                
                resolved_val_raw, _ = self._dynamic_service.evaluate(
                    expression=raw_hy_object_to_evaluate, context=context_for_dynamic_service,
                ...
                current_field_type = FieldType.from_string(type_from_schema) if type_from_schema else infer_type(resolved_python_value)
                
                final_attributes_for_model[attr_name] = TypedValue(
                    value=resolved_python_value, field_type=current_field_type,
                    is_dynamic=True, original=final_original_str,
                    item_type=initial_typed_value.item_type, enum_choices=initial_typed_value.enum_choices,
            ...
            self.logger.debug(f"  Processed instance attribute '{attr_name}'. Value: {repr(resolved_python_value)}")
        ...
        for schema_attr_name, attr_schema_details in entity_schema.items():
            ...
            default_value_from_schema = attr_schema_details.get("default_value") # This is the Python native value

            if "default_value" not in attr_schema_details:
                if str(attr_schema_details.get("required", "False")).lower() == "true":
                    self.logger.error(f"Missing required attribute '{schema_attr_name}' for '{definition_processing_key}' and no default provided.")
                continue
            
            original_for_default_tv = python_value_to_hy_repr_string_for_original(default_value_from_schema)
            self.logger.debug(f"  Applying default for schema attribute '{schema_attr_name}'. Default from schema: {repr(default_value_from_schema)}, Original str for TV: '{original_for_default_tv}'")
            ...
            if isinstance(default_value_from_schema, str) and default_value_from_schema.strip().startswith("("):
                ...
                try:
                    ...
                    self.logger.error(f"Could not rebuild HyExpr from list for default '{schema_attr_name}': {e_rebuild}. Treating as literal list.")
                    parsed_default_for_tv = default_value_from_schema
                    is_default_expr_dynamic = False
            ...
            if is_default_expr_dynamic:
                ...
                resolved_raw, _ = self._dynamic_service.evaluate(
                    expression=parsed_default_for_tv, context=context_for_dynamic_service,
            ...
            if declared_field_type in (FieldType.DATETIME, FieldType.ENTITY_REFERENCE) and resolved_default_python_value == "None":
            ...
            if is_default_expr_dynamic: # If default was an S-expr, re-infer from its resolved value
                ...
                if declared_field_type in (FieldType.DATETIME, FieldType.ENTITY_REFERENCE) and resolved_default_python_value is None:
                    final_field_type_for_default = declared_field_type
            ...
            final_attributes_for_model[schema_attr_name] = TypedValue(
                ...
                item_type=FieldType.from_string(attr_schema_details["item_type"]) if attr_schema_details.get("item_type") else None,
            ...
            self.logger.debug(f"  Applied default for '{schema_attr_name}'. Final TypedValue: {repr(final_attributes_for_model[schema_attr_name])}")

        return self._manager.create(
            name=entity_instance_name, entity_type=entity_type_name_str,
            category=category_name_str, attributes=final_attributes_for_model,
        )

    def process(self, nodes: List[HyNode], context: LoaderContext) -> Dict[str, Entity]:
        self.logger.debug(f"EntityLoader.process started. Context type: '{context.current_entity_type}', category: '{context.current_category}'")
        ...
        self.logger.debug(f"Parsed entity instance definitions: {list(definitions_by_processing_key.keys())}")
        ...
        for processing_key, properties_for_creation in definitions_by_processing_key.items():
            try:
                ...
                self.logger.error(f"Error creating/processing entity for key '{processing_key}': {e}", exc_info=True)
                raise 
        self.logger.debug(
