...
from utms.utms_types import FieldType, TypedValue, infer_type
...
class Config(ModelMixin):
    ...
    def set_dynamic_field(self, field_name: str, value: Any, original: str) -> None:
        ...
        field_type = (
            self.value.field_type if isinstance(self.value, TypedValue) else infer_type(value)
        )
        self.value = TypedValue(value, field_type, is_dynamic=True, original=original)
