from typing import Any, ClassVar, Dict, List, get_args, get_origin
...
class TimeEntity(TimeEntityProtocol):
    ...
    def get_all_attributes(cls) -> Dict[str, AttributeDefinition]:
        ...
        return {**cls.BASE_ATTRIBUTES, **cls.attributes}
    ...
    def _validate_type(self, value: Any, expected_type: type) -> bool:
        ...
        if origin is list:
            return isinstance(value, list) and all(
                isinstance(item, get_args(expected_type)[0]) for item in value
            )
        if origin is dict:
