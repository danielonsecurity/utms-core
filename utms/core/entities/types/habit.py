...
class HabitOccurrence:
    ...
    def __init__(
        self, timestamp: DecimalTimeStamp, value: Union[bool, int, float, Decimal], notes: str = ""
    ):
...
class Habit(TimeEntity):
    ...
    def get_today_total(self) -> Decimal:
        ...
        today_occurrences = [o for o in self.occurrences if o.timestamp.is_same_day(today)]
    ...
    def get_daily_average(self, days: int = 30) -> Decimal:
        ...
        seconds_in_day = day_unit.value
        recent_occurrences = [
            o for o in self.occurrences if (today - o.timestamp) / seconds_in_day <= days
        ]
