...
from utms.core.events.event import EventConfig, Event
...
def import_ics_calendar(ics_url: str, event_manager: EventManager) -> None:
    ...
    for component in cal.walk('VEVENT'):
        ...
        config = EventConfig(
            ...
            timerange={
                'start': Decimal(start.timestamp()),
                'end': Decimal(end.timestamp())
            },
            all_day=all_day,
            location=str(component.get('location', '')),
            url=str(component.get('url', '')),
            description=str(component.get('description', '')),
            created=Decimal(component.get('created').dt.timestamp()) if component.get('created') else None,
            modified=Decimal(component.get('last-modified').dt.timestamp()) if component.get('last-modified') else None,
            source=ics_url,
            properties={
...
def event_to_hy(event: Event) -> str:
    ...
    if event._clock_entries:
        clock_entries = python_to_hy(
            [[start, end] for start, end in event._clock_entries]
        )
