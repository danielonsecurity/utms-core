...
def node_to_typed_value(node: HyNode, field_type: Optional[FieldType] = None) -> TypedValue:
    ...
    # Create TypedValue
    return TypedValue(
        value=node.value, field_type=field_type, is_dynamic=node.is_dynamic, original=node.original
    )


def typed_value_to_node(
    typed_value: TypedValue, node_type: str = "value", field_name: Optional[str] = None
) -> HyNode:
