...
from ..resolvers.ast.node import HyNode
from ..utils import get_logger

T = TypeVar("T")  # The type of objects being loaded
M = TypeVar("M", bound=BaseManager)  # The manager type
...
class ComponentLoader(ABC, Generic[T, M]):
    ...
    def __init__(self, manager: M):
        ...
        self._logger = get_logger(f"loaders.{self.__class__.__name__.lower()}")
    ...
    def process(self, nodes: List[HyNode], context: LoaderContext) -> Dict[str, T]:
        ...
        """
        self._logger.debug("Starting processing with context: %s", context)

        try:
            # Parse nodes into intermediate definitions
            definitions = self.parse_definitions(nodes)
            self._logger.debug("Parsed definitions: %s", list(definitions.keys()))
            ...
            for label, properties in definitions.items():
                try:
                    ...
                    self._logger.error("Error creating object %s: %s", label, e)
                    raise

            # Load objects into manager
            self._manager.load(objects)

            return objects

        except Exception as e:
            self._logger.error("Error during processing: %s", e)
            raise
    ...
    def validate_node(self, node: HyNode, expected_type: str) -> bool:
        ...
        if node.type != expected_type:
            self._logger.debug("Skipping node of type %s (expected %s)", node.type, expected_type)
            return False
        return True
