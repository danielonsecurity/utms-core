        ...
        if isinstance(value, str) and value.startswith("("):
            try:
                ...
                config.config[key] = Config(
                    key=key, value=dynamic_info.latest_value, is_dynamic=True, original=value
            ...
            config.config.save()
            return {"value": value, "is_dynamic": False, "original": None}
...
async def evaluate_config_expression(
    ...
    try:
        if not isinstance(expression, str) or not expression.startswith("("):
            return {"value": expression, "is_dynamic": False, "original": None}

        # Only evaluate without saving
        resolved_value, dynamic_info = dynamic_resolution_service.evaluate(
            component_type="config", component_label=key, attribute="value", expression=expression
        )
