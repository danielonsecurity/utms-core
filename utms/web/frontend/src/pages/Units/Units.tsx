...
export const Units = () => {
	...
	.filter(([label, unit]) => {
	    ...
	    const matchesName = unit.name.toLowerCase().includes(nameSearch.toLowerCase());
	...
	.sort(([labelA, unitA], [labelB, unitB]) => {
	    switch(sortType) {
		case 'value-asc':
		    return new Decimal(unitA.value).minus(new Decimal(unitB.value)).toNumber();
		case 'value-desc':
		    return new Decimal(unitB.value).minus(new Decimal(unitA.value)).toNumber();
		case 'label-asc':
