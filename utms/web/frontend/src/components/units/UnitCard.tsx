...
export const UnitCard = ({ unit, label, onDelete, onUpdate, onGroupClick }: UnitCardProps) => {
    ...
    const handleContentEdit = (field: keyof typeof editedUnit, content: string) => {
	setEditedUnit(prev => ({
	    ...prev,
	    [field]: content
	}));
	...
	<div className="unit-card card" data-unit={label}>
        ...
        onInput={(e) => handleContentEdit('label', e.currentTarget.textContent || '')}
        ...
        onClick={startEditing}
            ...
            <div className={`unit-card__edit-controls ${isEditing ? 'visible' : ''}`}>
            <button 
